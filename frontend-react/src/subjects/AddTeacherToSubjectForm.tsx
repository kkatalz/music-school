...
import { useCreateSubject, useAddTeacherToSubject, useSubjects } from './useSubjects';
...
export const AddTeacherToSubject = () => {
  ...
  const handleSubmit = (e: FormEvent) => {
    e.preventDefault();
      const teacherId =  Number(formData.teacher);
      const subjectId =  Number(formData.subject);

    mutate({teacherId, subjectId});
  };

  return (
    <div className="p-4 sm:p-8">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gray-800">Add teacher to the subject</h1>
        <Link 
      ...
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-lg shadow-lg max-w-2xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="md:col-span-2">
            <label className="block text-gray-700 mb-1 font-semibold">Teacher</label>
            <select 
                name="teacher" 
