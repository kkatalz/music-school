...
import { useUpdateTeacher, useGetTeacherById } from "./useTeachers";
import type { FormEvent } from "react";
import type { UpdateTeacher } from "./teacher.types";
...
export const UpdateTeacherForm = () => {
    ...
    const initialTeacherData = useGetTeacherById(nteacherId);
  ...
  useEffect(() => {
    if (initialTeacherData) {
      setFormData({
        firstName: initialTeacherData.data?.firstName || '',
        lastName: initialTeacherData.data?.lastName || '',
        email: initialTeacherData.data?.email || '',
        phone: initialTeacherData.data?.phone || '',
        education: initialTeacherData.data?.education || '',
        isHeadTeacher: initialTeacherData.data?.isHeadTeacher || false,
      });
    }
  }, [initialTeacherData]); 


  const { mutate, isPending, isError, error } = useUpdateTeacher();
